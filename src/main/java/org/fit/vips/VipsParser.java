...
public class VipsParser {
	...
	public VipsParser(Viewport viewport) {
		...
		this._sizeTresholdHeight = 100;
		this._sizeTresholdWidth = 100;
	...
	public void parse()
	{
		if (_viewport != null)
		{
			...
			constructVipsBlockTree(_viewport.getElementBoxByName("body", true), _vipsBlocks);
	...
	{
		node.setBox(element);
		//System.out.println(element.getNode().getNodeName());
		if (! (element instanceof TextBox)&& element!=null)
		{
			ElementBox elementBox = (ElementBox) element;
            //System.out.println(elementBox.getElement().getTagName()+"   "+elementBox.getElement().getAttribute("class"));
			if (elementBox.getElement().getAttribute("id").contains("logo") ||elementBox.getElement().getAttribute("class").contains("logo")||
                    elementBox.getElement().getAttribute("id").contains("Logo") ||elementBox.getElement().getAttribute("class").contains("Logo"))
			{
			//	System.out.println(elementBox.getElement().getAttribute("class"));
			//	System.out.println("May be Logo");
				node.setMayBeLg(true);
			}
			if(node.isMayBeLg()){
				node.setIsVisualBlock(true);
				node.setDoC(11);
			}
			node.ImageUrl();
			for (Box box: elementBox.getSubBoxList())
			{
				VipsBlock child = new VipsBlock();
				if(node.isMayBeLg()){
					child.setMayBeLg(true);
					//child.setIsVisualBlock(true);
				 	//System.out.println("child is lg");
				}
                node.addChild(child);
