...
public final class VipsOutput {
	...
	public VipsOutput() {
		File imgFile = new File(folder+"/page.png");
		try {
			this._image = ImageIO.read(imgFile);
			this._pool = _image.createGraphics();
			_pool.setColor(Color.red);
		}catch (IOException e){
			System.out.print("constructer VipsOutput error");
		}
	...
	public void  VisualBlocksToImage(VisualStructure visualStructure)
	{

		String Id = visualStructure.getId();
		List<VipsBlock> boxs = visualStructure.getNestedBlocks();
		String fileName = folder+"/subPage/"+Id;
		String maybeLogo = "";
		try {
			if ( _pDoC < visualStructure.getDoC() || visualStructure.getChildrenVisualStructures().isEmpty()){

				List<String> ImageUrls = new ArrayList<>();
				for (VipsBlock vipsBlock : boxs) {
					vipsBlock.allImageUrl(ImageUrls);
				 }
				IsLogo isLogo = new IsLogo(false);
				for (VipsBlock vipsBlock : boxs) {
					vipsBlock.containLogo(isLogo);
					if(isLogo.get()){
						maybeLogo = maybeLogo+"logo";
						break;
					}
				}
				...
				if (!ImageUrls.isEmpty()) {
					...
					for(String imageurl:ImageUrls) {
						...
						if(!maybeLogo.equals(""))
							saveUrlImage(url, fileName + "/" + maybeLogo + tail);
					}
				}
			}
		...
		int x = visualStructure.getX();
		int y = visualStructure.getY();
		int width = visualStructure.getWidth();
		int height = visualStructure.getHeight();
		...
		try {
			if(!maybeLogo.equals("") && (width<=500 || height<=500)) {

				OperateImage operateImage = new OperateImage(x, y, width, height);
				String srcPath = folder + "/page.png";
				String outPath = fileName + maybeLogo + ".png";
				operateImage.setSrcpath(srcPath);
				operateImage.setSubpath(outPath);
				operateImage.cut();
			}
			_pool.draw(rect);
		} catch (IOException e) {
			System.out.println(" operateImage error ");
		}
		if (_pDoC >= visualStructure.getDoC()) {
			for(VisualStructure child:visualStructure.getChildrenVisualStructures())
			{
				VisualBlocksToImage(child);
			}
		}
	}
	...
	private void writeVisualBlocks(Element parentNode, VisualStructure visualStructure)
	{
		...
		if(!visualStructure.getNestedBlocks().isEmpty()) {
	...
	public void writeXML(VisualStructure visualStructure, Viewport pageViewport)
	{
		try
		{
			...
			else
			{
				...
				FileWriter fstream = new FileWriter(System.getProperty("user.dir")+"/"+_filename + ".xml");
	...
	public void saveUrlImage(URL url,String filename) {
		try {
