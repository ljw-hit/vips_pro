...
public class VipsBlock {
	...
	public void  ImageUrl(){
		    checkIsImg();
            if(_isImg){
				this.imageUrl = this.getElementBox().getElement().getAttribute("src");
	...
	/*get Block all image url*/
	public void allImageUrl(List<String> ImageUrl){
		if(isImg()){
		...
		if (!_children.isEmpty()){
			for(VipsBlock child : _children){
				child.allImageUrl(ImageUrl);
				//System.out.println(ImageUrl);
			}
		}
	}

    public void containLogo(IsLogo isLogo){
	    if(isLogo.get()){
	        return;
        }
		...
		if(isMayBeLg()) {
			...
			if (!_children.isEmpty()){
				for(VipsBlock child : _children){
					//System.out.println(child.containLogo());
					 child.containLogo(isLogo);
				}
			}
		}
	}
	...
	public boolean isMayBeLg() {
		return mayBeLg;
	}

	public void setMayBeLg(boolean mayBeLg) {
		this.mayBeLg = mayBeLg;
	}
}
